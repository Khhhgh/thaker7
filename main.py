...
-» ثم ارسل /start ✅ </strong>''',reply_markup=z,parse_mode='html')
      ...
      b = types.InlineKeyboardButton(text = "programmer 🇮🇶",url=f"t.me/PPGBB")
      aa.add(b)
      bot.send_message(message.chat.id,f"""*🌏Welcome to the bot* [{a}](tg://user?id={id})*
قسم الانستجرام /instagram
قسم التحميلات جميع المواقع /download

قسم صنع رابط مباشر لـ صورة،ملف،فيديو،مقطع صوتي  /urldownload

قسم اختصار رابط | مثال : رابط اندكس /urlcd

قسم زخرفة /zakrafa
قسم معلومات تيك توك /infoTiktok
قسم معلومات تويتر /infotwitter
قسم معلومات الفيزا /infoCard
قسم معلومات الـبين /infoBin*
